    ...
    for k2 = starting_point:sim.T
        ...
        if print == 1
            ...
            if k2 == sim.T
                if r{1} > 1
                    fprintf('\ntotal:               %3d       %3d         %3d', ...
                        sim.x.order, sim.x.deliver, sim.x.defrost);
                    fprintf('        %3d      %3d\n', ...
        ...
        matrix_data = [matrix_data; ...
            ...
            value_grill.grills, value_grill.sold, value_grill2.frozen_inventory, ...
            value_grill2.thawed_inventory, value_grill2.grilled_inventory ];
    end
end

matrix_data = [ matrix_data;
    sim.R, 0, sim.T, sim.x.order, sim.x.deliver, sim.x.defrost, ...
        sim.x.grill.grilled sim.x.grill.sold ];
output_filepath = fullfile(outputdir, 'final_data_burger_single_run.mat');
