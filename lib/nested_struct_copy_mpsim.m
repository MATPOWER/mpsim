function d = nested_struct_copy_mpsim(d, s, opt, parent)
%NESTED_STRUCT_COPY_MPSIM  Copies values from one nested struct to another.
%
%   DS = NESTED_STRUCT_COPY_MPSIM(D, S)
%   DS = NESTED_STRUCT_COPY_MPSIM(D, S, OPT)
%   DS = NESTED_STRUCT_COPY_MPSIM(D, S, OPT, PARENT)
%
%   DS = NESTED_STRUCT_COPY_MPSIM(D, S) copies values from a source struct S to
...
%   via a recursive call to NESTED_STRUCT_COPY_MPSIM.
%
%   Inputs:
...
%               ''          - (default) recursive call to nested_struct_copy_mpsim()
...
%       PARENT - cell array of parent field names used by
%               NESTED_STRUCT_COPY_MPSIM
...
%% form parent string
if DEBUG, fprintf('nested_struct_copy_mpsim() : parent = %s\n', strjoin(parent, '.')); end
...
for f = 1:length(fields)
    ...
    if ~isfield(valid_fields, ff)
        if check > 0        %% throw an error
            error('nested_struct_copy_mpsim: ''%s'' is not a valid field name', str);
    ...
    elseif isstruct(s.(ff)) && isempty(cm)
        %% assign via recursive call to nested_struct_copy_mpsim()
        if isfield(vf, ff)
